...
(defn- dashcard-with-param-mapping-to-venue-id [dashboard card]
  {:dashboard_id       (u/get-id dashboard)
   ...
   :parameter_mappings [{:parameter_id "242d5b8c",
                         :card_id      (u/get-id card)
...
(expect
 "Not found."
 (with-sharing-enabled-and-temp-card-referencing :venues :name [card]
   (http/client :get 404 (field-values-url card (data/id :venues :price)))))
...
(defmacro ^:private with-sharing-enabled-and-temp-dashcard-referencing
  {:style/indent 3}
...
(expect
 "Not found."
 (with-sharing-enabled-and-temp-dashcard-referencing :venues :name [dashboard]
   (http/client :get 404 (field-values-url dashboard (data/id :venues :price)))))
...
(expect
 "Invalid Request." ;; TODO - why is this a 400 invalid request ?
 (with-sharing-enabled-and-temp-card-referencing :venues :id [card]
...
(expect
 ...
 (with-sharing-enabled-and-temp-dashcard-referencing :venues :id [dashboard]
   (http/client :get #_200 (field-search-url dashboard (data/id :venues :id) (data/id :venues :name))
                :value "33 T")))
...
(expect
 "Not found."
 (with-sharing-enabled-and-temp-dashcard-referencing :venues :id [dashboard]
   (http/client :get 404 (field-search-url dashboard (data/id :venues :id) (data/id :venues :name))
                :value "33 T")))
